...
function nextTargetArea(code) {
  // Code Hanya Disini!
  const objKata = {TWO : 2,THREE: 3, FOUR: 4,SIX : 6,SEVEN: 7,EIGHT: 8}
  var arrCode = code.split('');
  var arrNew = [];
  for (var i= 0; i < arrCode.length; i++){
    for (var key in objKata){
      if (checkCode(arrCode[i], key)){
        arrNew.push(arrCode[i])
        arrCode.splice(i,1);
      }
    }
  }
...
function checkCode(kata1, kata2){
  for (var i = 0; i < kata2.length; i++){
    console.log(kata1, kata2[i]);
    if (kata1 !== kata2[i]){
      return false;
    }
  }
