...
function highestTopSpeed (data) {
  // your code here
  var arrPembalap = data.split(',');
  var arrSpeed = [];
  if (!data){
    return "No data";
  }
  for (var i = 0; i < arrPembalap.length; i++){
    arrSpeed.push(arrPembalap[i].split(':')[1]);
  }
  var topSpeed = 0
  for (var j = 0; j < arrSpeed.length; j++){
    if (parseInt(arrSpeed[j].split(' ')[0]) > topSpeed){
      topSpeed = parseInt(arrSpeed[j].split(' ')[0]);
    }
  }
  return topSpeed;
}
